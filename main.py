...
class VisionTag_Enterprise:
    def __init__(self, root):
        ...
        self.master_classes, self.class_colors, self.available_classes = [], {}, []
        self.current_index, self.history_tags = 0, {}
        self.ai_model, self.undo_stack, self.redo_stack = None, [], []
        self.toast_msg, self.toast_color, self.toast_timer, self.toast_step = "", "", None, 0
        
        self.source_dir = filedialog.askdirectory(title="Select Dataset Directory")
        if not self.source_dir: self.root.destroy(); return
            
        self.image_list = [f for f in os.listdir(self.source_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp'))]
        if not self.image_list: messagebox.showerror("Error", "No images found."); self.root.destroy(); return
        ...
        self.current_boxes, self.action_mode, self.selected_box_idx = [], "none", -1  
        self.clipboard_box, self.resize_axis = None, "none" 
        self.start_ox, self.start_oy, self.interact_start_state = 0, 0, []
        self.temp_rect, self.original_img = None, None
        self.zoom_level, self.img_x, self.img_y = 1.0, 0, 0
        self.last_mouse_x, self.last_mouse_y, self.current_mouse_x, self.current_mouse_y = 0, 0, 0, 0
        self.show_annotations, self.brightness, self.contrast = True, 1.0, 1.0
    ...
    def _is_matching_file(self, filename, base_name, extension=".txt"):
        ...
        if filename.endswith(target):
            ...
            if full_label_basename in getattr(self, 'raw_basenames', set()) and full_label_basename != base_name: return False
            prefix_len = len(filename) - len(target)
            if filename[prefix_len - 1] in ['-', '_']: return True
        return False
                    ...
                    if f.endswith('.txt'):
                        ...
                        if match_idx is None:
                            for raw_base, idx in img_map.items():
                                if self._is_matching_file(f, raw_base, ".txt"): match_idx = idx; break
                        if match_idx is not None:
                            if match_idx not in self.history_tags: self.history_tags[match_idx] = set()
    ...
    def _get_pure_basename(self, filename):
        ...
        for suf in ['_rot90', '_rot180', '_rot270', '_hflip', '_clahe', '_invert', '_bright', '_noise']:
            if base.endswith(suf): return base[:-len(suf)]
        return base
    ...
    def load_image(self):
        ...
        if 0 <= self.current_index < len(self.image_list):
            ...
            self.current_boxes.clear(); self.undo_stack.clear(); self.redo_stack.clear()
            self.selected_box_idx = -1; self.root.update_idletasks() 
            try:
                                    ...
                                    with open(os.path.join(lbl_dir, lbl_file), 'r') as f:
                                        for line in f:
                                            ...
                                            if len(parts) >= 9:
                                                ...
                                                ocx, ocy = (ox1+ox2+ox3+ox4)/4, (oy1+oy2+oy3+oy4)/4
                                                ow, oh = math.hypot(ox2-ox1, oy2-oy1), math.hypot(ox3-ox2, oy3-oy2)
                                                angle = math.degrees(math.atan2(oy2-oy1, ox2-ox1))
                                                self.current_boxes.append([cls, ocx, ocy, ow, oh, angle])
                                except Exception as e: print(f"Error reading {lbl_file}: {e}")
            except Exception as e: messagebox.showerror("Error", f"Gagal memuat gambar: {e}"); return
            cw, ch = self.canvas.winfo_width(), self.canvas.winfo_height()
            if cw <= 1: cw, ch = self.root.winfo_screenwidth() - 400, self.root.winfo_screenheight() - 200
            self.zoom_level, self.img_x, self.img_y = min(cw/img_w, ch/img_h) * 0.95, cw // 2, ch // 2
            ...
            if self.current_index in self.history_tags:
                self.save_indicator.config(text="✅", fg="#2ea44f"); self.history_label.config(text=f"Labeled in: {list(self.history_tags[self.current_index])}")
            else:
                self.save_indicator.config(text="⭕", fg="#555"); self.history_label.config(text="No labels yet.")
            self.show_image()
            self.render_tags()
            self.canvas.focus_set()

    def _remove_files_for_class(self, cls_name, filename, base_name):
        lbl_dir, img_dir = os.path.join(self.source_dir, cls_name, 'labels'), os.path.join(self.source_dir, cls_name, 'images')
        if os.path.exists(lbl_dir):
            for f in os.listdir(lbl_dir):
                if self._is_matching_file(f, base_name, ".txt"):
                    ...
                    if os.path.exists(os.path.join(img_dir, img_f)): os.remove(os.path.join(img_dir, img_f))

    def _cleanup_duplicates(self, pure_base, action):
        """Mendeteksi dan menghapus file augmentasi yang terduplikasi (lebih dari 1)"""
        expected_prefix = f"{pure_base}_{action}"
        duplicates = []
        
        for img_name in self.image_list:
            img_base = os.path.splitext(img_name)[0]
            # Mencari semua file yang berawalan nama original + jenis actionnya
            if img_base.startswith(expected_prefix):
                duplicates.append(img_name)
                
        if len(duplicates) > 1:
            # Urutkan berdasarkan panjang nama (img_rot90.jpg akan di depan img_rot90 (1).jpg)
            duplicates.sort(key=len)
            
            # Sisakan indeks ke-0, hapus sisanya
            for dup in duplicates[1:]:
                ...
                # 2. Hapus file label txt di dalam folder class
                ...
                # 3. Bersihkan memori aplikasi
                if dup_idx in self.history_tags:
                    del self.history_tags[dup_idx]
                self.image_list.remove(dup)
                
            # Kalibrasi ulang index memori agar tidak error setelah penghapusan
            self.scan_existing_labels()
    ...
    def delete_current_image(self):
        ...
        if messagebox.askyesno("Delete Image", f"Hapus file '{filename}' secara permanen beserta labelnya?"):
            ...
            if os.path.exists(img_path): os.remove(img_path)
            for cls in self.history_tags.get(self.current_index, set()): self._remove_files_for_class(cls, filename, base_name)
            if self.current_index in self.history_tags: del self.history_tags[self.current_index]
            del self.image_list[self.current_index]
            self.scan_existing_labels() 
            if self.current_index >= len(self.image_list): self.current_index = max(0, len(self.image_list) - 1)
            self.load_image(); self.set_status_message(f"Deleted: {filename}", "#ff4444")
    ...
    def final_save(self):
        ...
        if not self.current_boxes:
            if old_classes and messagebox.askyesno("Remove All Labels", "Hapus semua label untuk gambar ini?"):
                for cls in old_classes: self._remove_files_for_class(cls, filename, base_name)
                if self.current_index in self.history_tags: del self.history_tags[self.current_index]
            self.next_image(); return
        boxes_by_class = defaultdict(list)
        for box in self.current_boxes: boxes_by_class[box[0]].append(box)
        new_classes = set(boxes_by_class.keys())
        for cls_to_remove in old_classes - new_classes: self._remove_files_for_class(cls_to_remove, filename, base_name)
        for cls_name, boxes in boxes_by_class.items():
            ...
            if os.path.exists(lbl_dir):
                for existing in os.listdir(lbl_dir):
                    if self._is_matching_file(existing, base_name, ".txt"): target_basename = os.path.splitext(existing)[0]; break
            ...
            with open(os.path.join(class_dir, 'notes.json'), 'w') as f: json.dump({"categories": [{"id": 0, "name": cls_name}], "info": {"year": 2026, "version": "1.0", "contributor": "Label Studio"}}, f, indent=2)
            with open(os.path.join(lbl_dir, f"{target_basename}.txt"), 'w') as f:
                for box in boxes:
                    pts = get_rotated_points(box[1], box[2], box[3], box[4], box[5]); norm_pts = []
                    for px, py in pts: norm_pts.extend([max(0.0, min(1.0, px/img_w)), max(0.0, min(1.0, py/img_h))])
    ...
    def run_auto_detect(self):
        if not HAS_YOLO: messagebox.showerror("Dependency Missing", "Install via terminal:\npip install ultralytics"); return
        if self.ai_model is None:
            model_path = filedialog.askopenfilename(title="Select Trained YOLOv8 OBB Model (.pt)", filetypes=[("PyTorch Model", "*.pt")])
            if not model_path: return
            self.set_status_message("Loading AI Model...", "#ffd700", 3000); self.root.update()
            try: self.ai_model = YOLO(model_path)
            except Exception as e: messagebox.showerror("AI Error", f"Gagal meload model: {e}"); return
        filepath = os.path.join(self.source_dir, self.image_list[self.current_index])
        self.set_status_message("Running Inference...", "#ffd700", 2000); self.root.update()
        try:
            results = self.ai_model(filepath)
            self.push_state(); boxes_added = 0
            for r in results:
                if r.obb is not None:
                    for i in range(len(r.obb)):
                        ...
                        if cls_name not in self.master_classes:
                            self.master_classes.append(cls_name); self._save_external_config(); self._update_filter_combo_values()
                        self.current_boxes.append([cls_name, cx, cy, w, h, math.degrees(rad)]); boxes_added += 1
            self.render_tags(); self.show_image()
            self.set_status_message(f"AI Auto-Detect Selesai: {boxes_added} Objek ditemukan!", "#00ff00", 3000)
        except Exception as e: messagebox.showerror("Inference Error", f"Error:\n{e}")

    def show_bulk_rename_dialog(self):
        dialog = tk.Toplevel(self.root); dialog.title("📂 Bulk Rename Class"); dialog.geometry("400x250"); dialog.configure(bg="#1e1e1e"); dialog.transient(self.root); dialog.grab_set()
        ...
        old_class_var = tk.StringVar(); ttk.Combobox(dialog, textvariable=old_class_var, values=self.master_classes, state="readonly", font=("Segoe UI", 10)).pack(fill="x", padx=30, pady=(0, 15))
        tk.Label(dialog, text="Ketik Nama Class Baru:", bg="#1e1e1e", fg="#ccc", font=("Segoe UI", 9)).pack(anchor="w", padx=30)
        new_class_entry = tk.Entry(dialog, font=("Segoe UI", 10), bg="#2d2d2d", fg="white", insertbackground="white", borderwidth=1); new_class_entry.pack(fill="x", padx=30, pady=(0, 20), ipady=4)
        def execute():
            old_name, new_name = old_class_var.get(), new_class_entry.get().strip()
            if old_name and new_name and old_name != new_name:
                if messagebox.askyesno("Confirm", f"Ubah '{old_name}' menjadi '{new_name}'?"): self.execute_bulk_rename(old_name, new_name, dialog)
    ...
    def execute_bulk_rename(self, old_name, new_name, window):
        old_dir, new_dir = os.path.join(self.source_dir, old_name), os.path.join(self.source_dir, new_name)
        if os.path.exists(old_dir):
            if os.path.exists(new_dir): messagebox.showerror("Error", f"Folder tujuan '{new_name}' sudah ada!"); return
            os.rename(old_dir, new_dir)
            with open(os.path.join(new_dir, 'classes.txt'), 'w') as f: f.write(f"{new_name}\n")
            notes_json = os.path.join(new_dir, 'notes.json')
            if os.path.exists(notes_json):
                try:
                    with open(notes_json, 'r') as f: data = json.load(f)
                    if data.get('categories'): data['categories'][0]['name'] = new_name
                    with open(notes_json, 'w') as f: json.dump(data, f, indent=2)
                except: pass
        if old_name in self.master_classes:
            self.master_classes[self.master_classes.index(old_name)] = new_name
            self._save_external_config(); self._update_filter_combo_values()
        for tags in self.history_tags.values():
            if old_name in tags: tags.remove(old_name); tags.add(new_name)
        for box in self.current_boxes:
            if box[0] == old_name: box[0] = new_name
        self.render_tags(); self.show_image()
        if self.current_index in self.history_tags: self.history_label.config(text=f"Labeled in: [{', '.join(self.history_tags[self.current_index])}]")
        window.destroy(); messagebox.showinfo("Success", f"Berhasil merename '{old_name}' menjadi '{new_name}'!")

    def show_augmentation_dialog(self):
        if not self.history_tags and not self.current_boxes: 
            messagebox.showwarning("Warning", "Belum ada gambar yang dilabeli di dataset ini!"); return
        dialog = tk.Toplevel(self.root); dialog.title("🎛️ Augment Data (X-Ray Suite)"); dialog.geometry("500x480"); dialog.configure(bg="#1e1e1e"); dialog.transient(self.root); dialog.grab_set()
        ...
        # --- UI BARU: LISTBOX SPECIFIC CLASS ---
        list_frame = tk.Frame(dialog, bg="#1e1e1e"); list_frame.pack(fill="x", padx=50, pady=5)
        ...
        scrollbar = ttk.Scrollbar(list_frame, orient="vertical", command=self.class_listbox.yview); self.class_listbox.configure(yscrollcommand=scrollbar.set)
        self.class_listbox.pack(side="left", fill="x", expand=True); scrollbar.pack(side="right", fill="y")
        for cls in sorted(self.master_classes): self.class_listbox.insert(tk.END, cls)
        
        # Disable/Enable listbox berdasarkan pilihan Radiobutton
        def on_scope_change(*args):
            if self.aug_scope_var.get() == "specific": self.class_listbox.configure(state=tk.NORMAL)
            else: self.class_listbox.selection_clear(0, tk.END); self.class_listbox.configure(state=tk.DISABLED)
        self.aug_scope_var.trace("w", on_scope_change); on_scope_change()
        ...
        main_frame = tk.Frame(dialog, bg="#1e1e1e"); main_frame.pack(fill="x", padx=30, pady=5)
        ...
        def execute():
            ...
            if not actions: messagebox.showwarning("Warning", "Pilih teknik augmentasi!"); return
            
            scope = self.aug_scope_var.get(); targets = []
            ...
            if scope == "current": 
                ...
                if scope == "specific" and not sel_classes: messagebox.showwarning("Warning", "Pilih minimal satu class dari list!"); return
                
                for idx, tags in self.history_tags.items():
                    filename = self.image_list[idx]
                    
                    # 1. ORIGINAL PROTECTION: Hanya ambil gambar original (lewati jika merupakan file augmentasi)
                    if self._get_pure_basename(filename) != os.path.splitext(filename)[0]: continue 
                    
                    # 2. CLASS TARGETING: Cek jika scope specific
                    if scope == "specific" and not tags.intersection(sel_classes): continue
                    
                    # 3. SMART SKIP: Cek apakah gambar sudah pernah diproses/memiliki varian augmentasi
                    pure_base = self._get_pure_basename(filename)
                    if any(img != filename and self._get_pure_basename(img) == pure_base for img in self.image_list): continue
                    
                    targets.append(idx)
            
            if not targets:
                messagebox.showinfo("Info", "Tidak ada gambar target yang valid atau gambar sudah pernah diaugmentasi sebelumnya."); return
                
            target_filenames = [self.image_list[idx] for idx in targets]
            self.set_status_message(f"Processing & Cleaning {len(target_filenames)} images...", "#ffd700", 3000); self.root.update()
            
            success_count, deleted_dups_count = 0, 0
            
            for t_filename in target_filenames:
                if t_filename not in self.image_list: continue # Lewati jika terhapus
                pure_base = self._get_pure_basename(t_filename)
                
                for act in actions:
                    # 4. AUTO-DEDUPLICATION: Bersihkan duplikat terlebih dahulu
                    deleted_dups_count += self._cleanup_duplicates(pure_base, act)
                    
                    # Kalibrasi ulang index karena memory `image_list` berubah akibat cleanup
                    current_idx = self.image_list.index(t_filename)
                    
                    # Eksekusi Augmentasi (Otomatis Skip jika file _act sudah ada)
                    if self.execute_augmentation(act, current_idx): success_count += 1
    ...
    def execute_augmentation(self, action, target_idx):
        ...
        base_name, ext = os.path.splitext(filename)
        new_filename, new_basename = f"{base_name}_{action}{ext}", f"{base_name}_{action}"
        if new_filename in self.image_list: return False
        if target_idx == self.current_index:
            target_img, target_boxes = self.original_img, self.current_boxes
        else:
            filepath = os.path.join(self.source_dir, filename)
            if not os.path.exists(filepath): return False
            target_img, target_boxes = Image.open(filepath), []
                        ...
                        if self._is_matching_file(lbl_file, base_name, ".txt"):
                            with open(os.path.join(lbl_dir, lbl_file), 'r') as f:
                                for line in f:
                                    ...
                                    if len(parts) >= 9:
                                        nx1, ny1, nx2, ny2, nx3, ny3, nx4, ny4 = map(float, parts[1:9])
                                        ox1, oy1 = nx1*img_w, ny1*img_h; ox2, oy2 = nx2*img_w, ny2*img_h
                                        ox3, oy3 = nx3*img_w, ny3*img_h; ox4, oy4 = nx4*img_w, ny4*img_h
                                        target_boxes.append([cls, (ox1+ox2+ox3+ox4)/4, (oy1+oy2+oy3+oy4)/4, math.hypot(ox2-ox1, oy2-oy1), math.hypot(ox3-ox2, oy3-oy2), math.degrees(math.atan2(oy2-oy1, ox2-ox1))])
        if not target_boxes: return False 
        old_w, old_h = target_img.size; new_img, new_boxes = None, []

        if action.startswith('rot'):
            angle = int(action.replace('rot', ''))
            new_img = target_img.rotate(-angle, expand=True)
            for box in target_boxes:
                new_cx, new_cy = (old_h-box[2], box[1]) if angle==90 else (old_w-box[1], old_h-box[2]) if angle==180 else (box[2], old_w-box[1])
                new_boxes.append([box[0], new_cx, new_cy, box[3], box[4], (box[5] + angle) % 360])
        elif action == 'hflip':
            new_img = target_img.transpose(Image.FLIP_LEFT_RIGHT)
            for box in target_boxes: new_boxes.append([box[0], old_w - box[1], box[2], box[3], box[4], (180 - box[5]) % 360])
        elif action == 'clahe':
            if not HAS_CV2: messagebox.showerror("Error", "Library 'opencv-python' dibutuhkan untuk CLAHE."); return False
            img_cv = cv2.cvtColor(np.array(target_img.convert('RGB')), cv2.COLOR_RGB2LAB)
            img_cv[:,:,0] = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8)).apply(img_cv[:,:,0])
            new_img, new_boxes = Image.fromarray(cv2.cvtColor(img_cv, cv2.COLOR_LAB2RGB)), copy.deepcopy(target_boxes)
        elif action == 'invert':
            ...
            else: new_img = ImageOps.invert(target_img.convert('RGB'))
            new_boxes = copy.deepcopy(target_boxes)
        elif action == 'bright':
            new_img = ImageEnhance.Brightness(target_img).enhance(1.3); new_boxes = copy.deepcopy(target_boxes)
        elif action == 'noise':
            img_arr = np.array(target_img)
            new_img = Image.fromarray(np.clip(img_arr.astype('int16') + np.random.randint(-30, 30, img_arr.shape, dtype='int16'), 0, 255).astype('uint8'))
            new_boxes = copy.deepcopy(target_boxes)

        if not new_img: return False
        new_w, new_h = new_img.size
        new_img.save(os.path.join(self.source_dir, new_filename)); self.image_list.append(new_filename)
        ...
        for cls_name, boxes in boxes_by_class.items():
            ...
            with open(os.path.join(lbl_dir, f"{new_basename}.txt"), 'w') as f:
                for box in boxes:
                    pts = get_rotated_points(box[1], box[2], box[3], box[4], box[5]); norm_pts = []
                    for px, py in pts: norm_pts.extend([max(0.0, min(1.0, px/new_w)), max(0.0, min(1.0, py/new_h))])
                    f.write(f"0 " + " ".join(f"{p:.6f}" for p in norm_pts) + "\n")

        self.history_tags[len(self.image_list) - 1] = new_classes
        if target_idx == self.current_index: self.root.update()
        return True

    def load_external_config(self):
        config_path = os.path.join(self.source_dir, "classes.txt")
        if os.path.exists(config_path):
            with open(config_path, "r") as f: self.master_classes = [line.strip() for line in f if line.strip()]
        if not self.master_classes:
            self.master_classes = ['Accu', 'Aerosol', 'Alkohol', 'Bor Portable', 'Cairan Non MSDS', 'Cat', 'Catridge', 'Electronic Device', 'Freon', 'Handphone', 'Korek Api Zippo', 'Laptop', 'Lem', 'Medicine', 'Oli', 'Parfume', 'Pembersih', 'Petasan', 'Power Bank', 'Pupuk', 'Rokok', 'Serbuk', 'Tabung', 'Tanaman', 'Tinta', 'Live Animal', 'Kapasitor']
            self._save_external_config()

    def _save_external_config(self):
        with open(os.path.join(self.source_dir, "classes.txt"), "w") as f:
            for cls in self.master_classes: f.write(f"{cls}\n")

    def _update_filter_combo_values(self):
        if hasattr(self, 'filter_combo'):
            vals = ["All Classes"] + sorted(self.master_classes)
            self.filter_combo['values'] = vals
            if self.filter_class_var.get() not in vals: self.filter_class_var.set("All Classes")

    def _is_image_valid_for_filter(self, idx):
        if self.untagged_only_var.get() and idx in self.history_tags: return False
        filter_cls = getattr(self, 'filter_class_var', None)
        if filter_cls and filter_cls.get() != "All Classes":
            if idx not in self.history_tags or filter_cls.get() not in self.history_tags[idx]: return False
        return True

    def _on_untagged_toggle(self):
        if self.untagged_only_var.get(): self.filter_class_var.set("All Classes")
        self.apply_filter()

    def apply_filter(self, event=None):
        if self.filter_class_var.get() != "All Classes": self.untagged_only_var.set(False)
        if self._is_image_valid_for_filter(self.current_index): return
        for i in range(self.current_index + 1, len(self.image_list)):
            if self._is_image_valid_for_filter(i): self.current_index = i; self.load_image(); return
        for i in range(0, self.current_index):
            if self._is_image_valid_for_filter(i): self.current_index = i; self.load_image(); return
        messagebox.showinfo("Not Found", "Tidak ada gambar yang cocok dengan filter yang dipilih.")
        self.filter_class_var.set("All Classes"); self.untagged_only_var.set(False)
    ...
    def auto_skip_to_untagged(self):
        for i in range(len(self.image_list)):
            if i not in self.history_tags: self.current_index = i; break
        else: self.current_index = len(self.image_list) - 1 if self.image_list else 0
    ...
    def show_dashboard(self):
        ...
        # Dictionary terpisah untuk pelacakan presisi
        ...
        # 1. Hitung total gambar original fisik di folder
        for img in self.image_list:
            if self._get_pure_basename(img) == os.path.splitext(img)[0]: 
                total_original_in_dir += 1
                
        # 2. Kalkulasi Original vs Augmented dari data yang sudah dilabeli
        for idx, tags in self.history_tags.items():
            if idx >= len(self.image_list): continue
            filename = self.image_list[idx]
            
            # Deteksi apakah gambar ini Asli atau hasil Augmentasi
            ...
            # Distribusikan perhitungan ke masing-masing label
            for t in tags: 
                counts_total[t] += 1
                if is_orig:
                    counts_orig[t] += 1
        ...
        for i, cls_name in enumerate(sorted(counts_total.keys(), key=lambda x: counts_total[x], reverse=True)):
            ...
            # Diagram Bar dirender menggunakan Keseluruhan Data (Original + Augmented)
            ...
            # Menampilkan detail rincian Original dan Augmented per Label
    ...
    def set_status_message(self, text, color="#00ff00", duration=2000):
        ...
        self.toast_msg, self.toast_color, self.toast_step = text, color, 0; self.show_image() 
        if hasattr(self, 'toast_timer') and self.toast_timer: self.root.after_cancel(self.toast_timer)
        self.toast_timer = self.root.after(duration, self.fade_out_toast)

    def fade_out_toast(self):
        self.toast_step += 1
        if self.toast_step > 10: self.toast_msg = ""; self.show_image()
        else: self.show_image(); self.toast_timer = self.root.after(40, self.fade_out_toast)

    def push_state(self):
        self.undo_stack.append(copy.deepcopy(self.current_boxes)); self.redo_stack.clear()

    def perform_undo(self):
        if self.undo_stack:
            self.redo_stack.append(copy.deepcopy(self.current_boxes)); self.current_boxes = self.undo_stack.pop()
            self.selected_box_idx = -1; self.show_image(); self.set_status_message("UNDO successful", "#ffd700")

    def perform_redo(self):
        if self.redo_stack:
            self.undo_stack.append(copy.deepcopy(self.current_boxes)); self.current_boxes = self.redo_stack.pop()
            self.selected_box_idx = -1; self.show_image(); self.set_status_message("REDO successful", "#00f2ff")

    def copy_box(self):
        if self.selected_box_idx != -1 and self.selected_box_idx < len(self.current_boxes):
            self.clipboard_box = list(self.current_boxes[self.selected_box_idx]); self.set_status_message(f"COPIED: {self.clipboard_box[0]}", "#00ff00")
        else: self.set_status_message("Select a box first to copy!", "#ff4444")

    def paste_box(self):
        if self.clipboard_box:
            self.push_state(); new_box = list(self.clipboard_box); ox, oy = self.canvas_to_img(self.current_mouse_x, self.current_mouse_y)
            new_box[1], new_box[2] = ox, oy; self.current_boxes.append(new_box); self.selected_box_idx = len(self.current_boxes) - 1 
            self.show_image(); self.set_status_message("PASTED!", "#00ff00")
        else: self.set_status_message("Clipboard empty!", "#ff4444")

    def toggle_visibility(self): self.show_annotations = not self.show_annotations; self.show_image()

    def adjust_enhancement(self, target, amount):
        if target == 'brightness': self.brightness = max(0.2, min(3.0, self.brightness + amount))
        elif target == 'contrast': self.contrast = max(0.2, min(3.0, self.contrast + amount))
        self.show_image()

    def _on_tag_scroll(self, event): self.tag_canvas.yview_scroll(int(-1*(event.delta/120)), "units")
    ...
    def render_tags(self):
        ...
        for cls in sorted(filtered):
            row = tk.Frame(self.tag_inner, bg="#1e1e1e"); row.pack(fill="x", pady=3)
    ...
    def delete_master_class(self, cls_name):
        if messagebox.askyesno("Delete Class", f"Hapus '{cls_name}' dari daftar Master Class?"):
            if cls_name in self.master_classes:
                self.master_classes.remove(cls_name); self._save_external_config(); self.render_tags(); self._update_filter_combo_values()
                if self.get_active_class() == cls_name: self.queue_list.delete(0, tk.END); self.selected_box_idx = -1; self.show_image()
            self.set_status_message(f"Class '{cls_name}' deleted!", "#ff4444")

    def set_active_class(self, tag):
        self.queue_list.delete(0, tk.END); self.queue_list.insert(tk.END, tag)
        if self.selected_box_idx != -1 and self.selected_box_idx < len(self.current_boxes):
            self.push_state(); self.current_boxes[self.selected_box_idx][0] = tag; self.show_image(); self.set_status_message(f"Label updated to {tag}", "#00ff00")
        self.canvas.focus_set() 

    def get_active_class(self):
        items = self.queue_list.get(0, tk.END); return items[0] if items else None

    def add_to_master(self, event=None):
        val = self.search_var.get().strip()
        if val and val not in self.master_classes:
            self.master_classes.append(val); self._save_external_config(); self.search_var.set(""); self.render_tags(); self._update_filter_combo_values()
        self.canvas.focus_set(); return "break"

    def jump_to_image(self, event=None):
        query = self.jump_var.get().strip().lower()
        if not query: self.canvas.focus_set(); return "break"
        if query.isdigit() and 0 <= int(query)-1 < len(self.image_list):
            self.current_index = int(query)-1; self.load_image()
        else:
            for i, filename in enumerate(self.image_list):
                if query in filename.lower(): self.current_index = i; self.load_image(); self.canvas.focus_set(); return "break"
            messagebox.showinfo("Not Found", f"No image found containing: '{query}'")
        self.canvas.focus_set(); return "break"

    def canvas_to_img(self, cx, cy):
        w_disp, h_disp = self.original_img.size[0] * self.zoom_level, self.original_img.size[1] * self.zoom_level
        return (cx - (self.img_x - w_disp / 2)) / self.zoom_level, (cy - (self.img_y - h_disp / 2)) / self.zoom_level

    def img_to_canvas(self, ox, oy):
        w_disp, h_disp = self.original_img.size[0] * self.zoom_level, self.original_img.size[1] * self.zoom_level
        return (ox * self.zoom_level) + (self.img_x - w_disp / 2), (oy * self.zoom_level) + (self.img_y - h_disp / 2)

    def update_crosshair(self, event):
        self.current_mouse_x, self.current_mouse_y = event.x, event.y
        ...
        if ctrl_pressed:
            ...
            for i in reversed(range(len(self.current_boxes))):
                if is_point_in_box(ox, oy, self.current_boxes[i]):
                    self.push_state(); rotated_box = self.current_boxes.pop(i); self.current_boxes.append(rotated_box)
                    self.selected_box_idx = len(self.current_boxes) - 1
                    self.current_boxes[-1][5] = (self.current_boxes[-1][5] + (5 if delta > 0 else -5)) % 360
                    self.show_image(); break
        else:
            self.zoom_level *= (1.1 if delta > 0 else 0.9); self.show_image()

    def start_drag(self, event): self.canvas.focus_set(); self.last_mouse_x, self.last_mouse_y = event.x, event.y
    def do_drag(self, event): self.img_x += event.x - self.last_mouse_x; self.img_y += event.y - self.last_mouse_y; self.last_mouse_x, self.last_mouse_y = event.x, event.y; self.show_image()

    def start_interaction(self, event):
        self.canvas.focus_set(); ox, oy = self.canvas_to_img(event.x, event.y)
        if event.state & 0x0001: 
            for i in reversed(range(len(self.current_boxes))):
                if is_point_in_box(ox, oy, self.current_boxes[i]):
                    self.push_state(); self.current_boxes.pop(i)
                    if self.selected_box_idx == i: self.selected_box_idx = -1
                    self.show_image(); break
            self.action_mode = "none"; return

        for i in reversed(range(len(self.current_boxes))):
            box = self.current_boxes[i]
            if is_point_in_box(ox, oy, box):
                self.push_state(); clicked_box = self.current_boxes.pop(i); self.current_boxes.append(clicked_box)
                self.selected_box_idx = len(self.current_boxes) - 1  
                cls, cx, cy, w, h, angle = clicked_box
                lx, ly = get_local_coords(ox, oy, cx, cy, angle)
                self.queue_list.delete(0, tk.END); self.queue_list.insert(tk.END, cls)
                margin = 15 / self.zoom_level 
                is_edge_x, is_edge_y = abs(lx) > (w/2 - margin), abs(ly) > (h/2 - margin)
                self.start_ox, self.start_oy, self.interact_start_state = ox, oy, [cx, cy, w, h, angle]
                if is_edge_x and is_edge_y: self.action_mode, self.resize_axis = "resize", "both"
                elif is_edge_x: self.action_mode, self.resize_axis = "resize", "width"
                elif is_edge_y: self.action_mode, self.resize_axis = "resize", "height"
                else: self.action_mode = "move"
                self.show_image(); return

        self.selected_box_idx = -1; self.show_image()
        if not self.get_active_class():
            messagebox.showwarning("No Class", "Please select an Active Class from the right panel first."); self.action_mode = "none"; return
        self.action_mode, self.start_x, self.start_y = "draw", event.x, event.y
    ...
    def do_interaction(self, event):
        ...
        if self.action_mode == "move":
            ...
            if self.resize_axis in ("both", "width"): self.current_boxes[self.selected_box_idx][3] = max(10, abs(lx) * 2)
            if self.resize_axis in ("both", "height"): self.current_boxes[self.selected_box_idx][4] = max(10, abs(ly) * 2)
            self.show_image()
    ...
    def end_interaction(self, event):
        if self.action_mode == "draw":
            ...
            ox1, oy1 = self.canvas_to_img(self.start_x, self.start_y); ox2, oy2 = self.canvas_to_img(event.x, event.y)
            ox1, ox2 = min(ox1, ox2), max(ox1, ox2); oy1, oy2 = min(oy1, oy2), max(oy1, oy2)
            ow, oh = ox2 - ox1, oy2 - oy1
            if ow > 5 and oh > 5:
                self.push_state(); self.current_boxes.append([self.get_active_class(), ox1 + ow / 2, oy1 + oh / 2, ow, oh, 0.0])
                self.selected_box_idx = len(self.current_boxes) - 1
            self.show_image()
        self.action_mode = "none"

    def clear_all_boxes(self):
        self.push_state(); self.current_boxes.clear(); self.selected_box_idx = -1; self.show_image()
    ...
    def draw_all_boxes(self):
        ...
        for i, box in enumerate(self.current_boxes):
            ...
            pts = get_rotated_points(ocx, ocy, ow, oh, angle); c_pts = []
            for ox, oy in pts: cx, cy = self.img_to_canvas(ox, oy); c_pts.extend([cx, cy])
    ...
    def show_image(self):
        ...
        if self.brightness != 1.0: resized = ImageEnhance.Brightness(resized).enhance(self.brightness)
        if self.contrast != 1.0: resized = ImageEnhance.Contrast(resized).enhance(self.contrast)
        ...
        pad_x, pad_y, hud_font = 12, 10, ("Consolas", 9, "bold") 
        ...
        if bbox_hud:
            ...
            self.canvas.tag_lower(acc_hud, t_hud); self.canvas.tag_lower(bg_hud, acc_hud)
        ...
        if hasattr(self, 'toast_msg') and self.toast_msg:
            ...
            text_color, bg_color, outline_color, acc_color = fade_hex_color("#ffffff", step), fade_hex_color("#1c1c1c", step), fade_hex_color("#333333", step), fade_hex_color(self.toast_color, step)
            ...
            if bbox_toast:
                ...
                self.canvas.tag_lower(acc_toast, t_toast); self.canvas.tag_lower(bg_toast, acc_toast)
        
        self.canvas.itemconfig("crosshair", state="normal" if self.show_annotations else "hidden")
        if self.show_annotations: self.canvas.tag_raise("crosshair")
        self.canvas.tag_raise("hud")

    def setup_shortcuts(self):
        def is_not_typing(e): return e.widget.winfo_class() != 'Entry'
    ...
    def setup_ui(self):
        self.main_container = tk.Frame(self.root, bg="#121212"); self.main_container.pack(fill="both", expand=True)
        self.sidebar = tk.Frame(self.main_container, bg="#1e1e1e", width=380); self.sidebar.pack(side="right", fill="y"); self.sidebar.pack_propagate(False)

        top_btns = tk.Frame(self.sidebar, bg="#1e1e1e"); top_btns.pack(fill="x", padx=20, pady=5)
        ...
        action_frame = tk.Frame(self.sidebar, bg="#1e1e1e"); action_frame.pack(fill="x", padx=20, pady=5)
        ...
        self.queue_list = tk.Listbox(self.sidebar, height=1, bg="#121212", fg="#00f2ff", borderwidth=0, highlightthickness=1, font=("Segoe UI", 12, "bold")); self.queue_list.pack(fill="x", padx=20, pady=5)
        
        tk.Label(self.sidebar, text="STATUS", bg="#1e1e1e", fg="#ffd700", font=("Segoe UI", 8, "bold")).pack(anchor="w", padx=20, pady=(0, 2))
        self.history_container = tk.Frame(self.sidebar, bg="#1e1e1e"); self.history_container.pack(fill="x", padx=20, pady=5)
        self.history_label = tk.Label(self.history_container, text="", bg="#1e1e1e", fg="#ffd700", font=("Segoe UI", 9, "italic"), justify="left", anchor="nw"); self.history_label.pack(fill="both", expand=True)

        tk.Label(self.sidebar, text="SEARCH & SELECT CLASS", bg="#1e1e1e", fg="#888", font=("Segoe UI", 8, "bold")).pack(anchor="w", padx=20, pady=(0, 5))
        search_frame = tk.Frame(self.sidebar, bg="#1e1e1e"); search_frame.pack(fill="x", padx=20, pady=5)
        self.search_var = tk.StringVar(); self.search_var.trace("w", lambda n, i, m: self.render_tags())
        self.search_entry = tk.Entry(search_frame, textvariable=self.search_var, bg="#2d2d2d", fg="white", insertbackground="white", borderwidth=0, font=("Segoe UI", 11)); self.search_entry.pack(side="left", fill="both", expand=True, padx=(0, 6))
        ...
        btn_frame = tk.Frame(self.sidebar, bg="#1e1e1e"); btn_frame.pack(fill="x", side="bottom", padx=20, pady=5)
        self.btn_delete_img = tk.Button(btn_frame, text="🗑️ DELETE IMAGE", bg="#8B0000", fg="white", relief="flat", font=("Segoe UI", 8, "bold"), pady=3, command=self.delete_current_image); self.btn_delete_img.pack(fill="x", pady=(0, 6))
        self.btn_clear = tk.Button(btn_frame, text="CLEAR ALL LABELS", bg="#ff4444", fg="white", relief="flat", font=("Segoe UI", 8, "bold"), pady=3, command=self.clear_all_boxes); self.btn_clear.pack(fill="x", pady=(0, 6))
        self.btn_save = tk.Button(btn_frame, text="CONFIRM & EXPORT (ENTER)", bg="#2ea44f", fg="white", relief="flat", font=("Segoe UI", 9, "bold"), pady=4, command=self.final_save); self.btn_save.pack(fill="x")

        self.list_container = tk.Frame(self.sidebar, bg="#1e1e1e"); self.list_container.pack(fill="both", expand=True, padx=20, pady=5)
        self.tag_canvas = tk.Canvas(self.list_container, bg="#1e1e1e", highlightthickness=0); self.tag_scroll = ttk.Scrollbar(self.list_container, orient="vertical", command=self.tag_canvas.yview); self.tag_inner = tk.Frame(self.tag_canvas, bg="#1e1e1e")
        self.tag_canvas.create_window((0, 0), window=self.tag_inner, anchor="nw", width=320); self.tag_canvas.configure(yscrollcommand=self.tag_scroll.set)
        self.tag_canvas.bind("<Enter>", lambda e: self.tag_canvas.bind_all("<MouseWheel>", self._on_tag_scroll)); self.tag_canvas.bind("<Leave>", lambda e: self.tag_canvas.unbind_all("<MouseWheel>"))
        self.tag_canvas.pack(side="left", fill="both", expand=True); self.tag_scroll.pack(side="right", fill="y"); self.tag_inner.bind("<Configure>", lambda e: self.tag_canvas.configure(scrollregion=self.tag_canvas.bbox("all")))

        self.viewer_frame = tk.Frame(self.main_container, bg="#121212"); self.viewer_frame.pack(side="left", fill="both", expand=True, padx=10, pady=10)
        self.canvas = tk.Canvas(self.viewer_frame, bg="#000000", highlightthickness=0, cursor="crosshair"); self.canvas.pack(fill="both", expand=True)
        self.vline = self.canvas.create_line(0, 0, 0, 0, fill="#00f2ff", dash=(4, 4), tags="crosshair"); self.hline = self.canvas.create_line(0, 0, 0, 0, fill="#00f2ff", dash=(4, 4), tags="crosshair")
        self.canvas.bind("<Motion>", self.update_crosshair); self.canvas.bind("<MouseWheel>", self.handle_scroll); self.canvas.bind("<ButtonPress-3>", self.start_drag); self.canvas.bind("<B3-Motion>", self.do_drag)
        self.canvas.bind("<ButtonPress-1>", self.start_interaction); self.canvas.bind("<B1-Motion>", self.do_interaction); self.canvas.bind("<ButtonRelease-1>", self.end_interaction)

        self.nav_info_bar = tk.Frame(self.viewer_frame, bg="#1e1e1e", height=50); self.nav_info_bar.pack(fill="x", pady=(5,0))
        ...
        self.status_text = tk.Label(self.nav_info_bar, text="", bg="#1e1e1e", fg="#888", font=("Consolas", 10)); self.status_text.pack(side="right", padx=10)

        info_mid = tk.Frame(self.nav_info_bar, bg="#1e1e1e"); info_mid.pack(side="left", fill="x", expand=True)
        ...
        self.jump_entry.pack(side="left", padx=10, ipady=3); self.jump_entry.bind("<Return>", self.jump_to_image)
        ...
        self.filter_combo = ttk.Combobox(info_mid, textvariable=self.filter_class_var, state="readonly", width=15, font=("Segoe UI", 9)); self.filter_combo.pack(side="left", padx=(0, 10)); self.filter_combo.bind("<<ComboboxSelected>>", self.apply_filter)
        self._update_filter_combo_values()
        
        self.save_indicator = tk.Label(info_mid, text="", bg="#1e1e1e", font=("Segoe UI", 12)); self.save_indicator.pack(side="left", padx=2)
        self.filename_label = tk.Label(info_mid, text="", bg="#1e1e1e", fg="#00f2ff", font=("Segoe UI", 9, "bold"), anchor="w"); self.filename_label.pack(side="left", fill="x", expand=True)
